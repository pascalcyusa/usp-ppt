...
from rclpy.executors import SingleThreadedExecutor # Keep for potential future use of actions
from rclpy.qos import qos_profile_sensor_data # For sensor subscriptions if needed later
...
if not all([AIRTABLE_API_TOKEN, AIRTABLE_BASE_ID, AIRTABLE_TABLE_NAME]):
    raise EnvironmentError("Missing required Airtable environment variables. Please check your .env file.")
...
AIRTABLE_COOKING_2_STATUS_FIELD = "Cooking 2 Status" # Assuming a second cooking step/station exists
...
STATION_COLORS_HSV = {
    ...
    0: {"name": "Pickup Station", "hsv_lower": (100, 100, 50), "hsv_upper": (130, 255, 255), "color_bgr": (255, 0, 0)}, # Example: Blue

    # Index 1: Cooking Station 1 (Batter/Cook)
    1: {"name": "Cooking 1", "hsv_lower": (0, 100, 100), "hsv_upper": (10, 255, 255), "color_bgr": (0, 0, 255)},   # Example: Red
    ...
    2: {"name": "Chocolate Chips", "hsv_lower": (20, 100, 100), "hsv_upper": (40, 255, 255), "color_bgr": (0, 255, 255)}, # Example: Yellow

    # Index 3: Whipped Cream Station
    3: {"name": "Whipped Cream", "hsv_lower": (50, 100, 50), "hsv_upper": (80, 255, 255), "color_bgr": (0, 255, 0)},   # Example: Green

    # Index 4: Sprinkles Station
    4: {"name": "Sprinkles", "hsv_lower": (140, 100, 50), "hsv_upper": (170, 255, 255), "color_bgr": (255, 0, 255)},  # Example: Magenta/Pink
}
NUM_STATIONS_PHYSICAL = len(STATION_COLORS_HSV) # Total physical stations with markers
...
STATION_FIELD_TO_INDEX = {
    ...
    AIRTABLE_CHOCOLATE_CHIPS_STATUS_FIELD: 2, # Map Choc Chips Status field to index 2
    AIRTABLE_WHIPPED_CREAM_STATUS_FIELD: 3,   # Map WC Status field to index 3
    AIRTABLE_SPRINKLES_STATUS_FIELD: 4,       # Map Sprinkles Status field to index 4
...
IR_POLL_RATE = 0.02             # Seconds (50 Hz) - How often to check IR sensors
COLOR_POLL_RATE = 0.1           # Seconds (10 Hz) - How often to check camera for colors
AIRTABLE_POLL_RATE = 2.0        # Seconds - How often to check Airtable for '99' status
...
COLOR_DETECTION_THRESHOLD = 1500 # Min pixels of target color to trigger detection (tune this!)
...
class RobotState(Enum):
    ...
    ORDER_COMPLETE = auto()         # Intermediate state after finishing all steps for one order
...
class PancakeRobotNode(Node):
    def __init__(self):
        ...
        self.current_order = None # Stores details: {'record_id': ..., 'order_name': ..., 'required_stations': [...], 'station_status': {...}}
        self.station_sequence = [] # Ordered list of station indices to visit for the current order [1, 3, 4, 0]
        self.current_sequence_index = 0 # Index into self.station_sequence
        self.target_station_index = -1 # Physical station index we are currently moving towards/at
        ...
        self.control_timer_period = 0.05 # Seconds (20 Hz) - Should be fast enough for IR + Color checks
    ...
    def check_for_station_color(self, frame, target_idx):
        ...
        color_bgr = color_info.get("color_bgr", (255, 255, 255)) # Default to white if not defined
        ...
        try:
            ...
            if detected_pixels > COLOR_DETECTION_THRESHOLD and \
               (current_time - self.last_color_detection_times.get(target_idx, 0.0) > COLOR_COOLDOWN_SEC):
                self.get_logger().info(f"Detected {target_color_name} (Index {target_idx})!")
                self.last_color_detection_times[target_idx] = current_time # Update last detected time
    ...
    def update_station_status_in_airtable(self, record_id, station_field_name, new_status_code):
        ...
        if not record_id or not station_field_name:
            self.get_logger().error("Cannot update Airtable: record_id or station_field_name missing.")
            return False

        self.get_logger().info(f"Updating Airtable: Record {record_id}, Field '{station_field_name}' to {new_status_code}")
        ...
        try:
            ...
            if self.current_order and self.current_order["record_id"] == record_id:
                 if station_field_name in self.current_order["station_status"]:
                     ...
                     self.get_logger().debug(f"Local order status cache updated for {station_field_name}.")
            return True
        except requests.exceptions.RequestException as req_err:
            self.log_airtable_error(f"update field {station_field_name}", req_err)
            return False
        except Exception as e:
            self.get_logger().error(f"Unexpected error during Airtable update: {e}")
            return False
    ...
    def check_station_status_in_airtable(self, record_id, station_field_name):
        ...
        if not record_id or not station_field_name:
            self.get_logger().error("Cannot check Airtable status: record_id or station_field_name missing.")
            return None # Return None to indicate check failure

        self.get_logger().debug(f"Checking Airtable: Record {record_id}, Field '{station_field_name}' status...")
        check_url = f"{AIRTABLE_URL}/{record_id}"
        params = {"fields[]": [station_field_name]} # Only request the specific field
        ...
        try:
            ...
            status_value = fields.get(station_field_name) # Will be None if field doesn't exist or isn't set

            if status_value is not None:
                self.get_logger().debug(f"Airtable check successful: Status for '{station_field_name}' is {status_value}.")
                # Update local cache
                if self.current_order and self.current_order["record_id"] == record_id:
                    if station_field_name in self.current_order["station_status"]:
                ...
                return status_value
                 ...
                 self.get_logger().warn(f"Field '{station_field_name}' not found or not set for record {record_id}.")
                 return None # Explicitly return None if field is missing/null

        except requests.exceptions.RequestException as req_err:
            self.log_airtable_error(f"check status for {station_field_name}", req_err)
            return None
        except Exception as e:
            self.get_logger().error(f"Unexpected error during Airtable status check: {e}")
            return None
    ...
    def start_airtable_polling(self):
        ...
        self.get_logger().info(f"Starting Airtable polling timer (every {AIRTABLE_POLL_RATE}s) for station {self.target_station_index} completion.")
    ...
    def airtable_poll_callback(self):
        ...
        if not self.current_order or self.target_station_index < 0:
            self.get_logger().error("Polling error: No current order or invalid target station index.")
        ...
        if not target_field:
            self.get_logger().error(f"Polling error: No Airtable field mapped for station index {self.target_station_index}.")
            self.state = RobotState.ERROR
            self.stop_airtable_polling()
            self.stop_moving()
            return
        ...
        current_status = self.check_station_status_in_airtable(self.current_order["record_id"], target_field)

        if current_status == STATUS_DONE:
            self.get_logger().info(f"Station {self.target_station_index} ({target_field}) reported DONE (99).")
            ...
            # State likely set to AIRTABLE_ERROR by the check function. Stop polling.
             self.get_logger().error(f"Error checking status for {target_field}. Stopping polling.")
             self.stop_airtable_polling()
             # State should already be AIRTABLE_ERROR

        else:
            # Status is not DONE (could be 0, 1, or some other intermediate value)
            self.get_logger().debug(f"Waiting for {target_field} status {STATUS_DONE}, currently {current_status}.")
            # Check for timeout
            elapsed_time = time.time() - self.wait_start_time
            if elapsed_time > STATION_WAIT_TIMEOUT_SEC:
                self.get_logger().error(f"TIMEOUT: Station {self.target_station_index} ({target_field}) did not reach status {STATUS_DONE} within {STATION_WAIT_TIMEOUT_SEC} seconds.")
                self.play_sound([(300, 500), (250, 500)]) # Timeout sound
                self.stop_airtable_polling()
                self.update_station_status_in_airtable(self.current_order["record_id"], target_field, -1) # Mark as failed (-1 or similar)
        ...
        if current_state == RobotState.IDLE:
            ...
            if fetched_order:
                ...
                if self.pancakes_made_count > 0:
                     self.play_sound([(600,100), (700,100), (800, 300)]) # Batch complete sound
                     self.get_logger().info(f"Completed {self.pancakes_made_count} order(s) this run.")
                else:
                     self.play_sound([(400, 500)]) # No orders found initially sound
            ...
            if not self.current_order:
                ...
                for req_field, station_idx in TOPPING_REQ_FIELD_TO_INDEX.items():
                    if self.current_order["requirements"].get(req_field, 0) > 0:
                         ...
                         self.get_logger().info(f" - Adding Station {station_idx} ({STATION_COLORS_HSV[station_idx]['name']})")
                # Always end by returning to the Pickup Station
                self.station_sequence.append(0)

                self.get_logger().info(f"Planned route (indices): {self.station_sequence}")
                ...
                if not self.station_sequence:
                    ...
                    self.get_logger().info(f"First target station: {self.target_station_index} ({STATION_COLORS_HSV[self.target_station_index]['name']})")
            ...
            detected, debug_frame = self.check_for_station_color(frame, self.target_station_index)
            if detected:
                station_name = STATION_COLORS_HSV[self.target_station_index]['name']
                self.get_logger().info(f"Target color marker for {station_name} (Index {self.target_station_index}) DETECTED!")
                ...
                if left_ir == GPIO.HIGH and right_ir == GPIO.HIGH:
                    ...
                    self.get_logger().warn("IR: Both sensors on black (Line Lost/Junction?). Rotating slightly.")
                    linear_speed = 0.0
                    angular_speed = -LOST_LINE_ROTATE_SPEED # Rotate right to try and find it
                     ...
                     self.get_logger().error(f"IR: Unexpected sensor state L={left_ir} R={right_ir}. Stopping.")
                     linear_speed = 0.0
                     angular_speed = 0.0
                     next_state = RobotState.ERROR
            ...
            if self.debug_windows and debug_frame is not None:
                 ...
                 cv2.putText(debug_frame, ir_text, (frame.shape[1] - 150, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2) # Black text
            ...
            if not station_field:
                 self.get_logger().error(f"Arrival error: No Airtable field for station index {self.target_station_index}.")
                 next_state = RobotState.ERROR
            else:
                 self.get_logger().info(f"Arrived at {STATION_COLORS_HSV[self.target_station_index]['name']} ({station_field}). Updating status to {STATUS_ARRIVED}.")
                 if self.update_station_status_in_airtable(self.current_order["record_id"], station_field, STATUS_ARRIVED):
                     ...
                     self.get_logger().error(f"Failed to update Airtable status upon arrival at {station_field}.")
                     # State should already be AIRTABLE_ERROR
             ...
             if self.airtable_poll_timer is None: # Timer stopped itself (found 99 or timed out)
                 ...
                 # Check for other errors that might stop polling
                     ...
                     self.get_logger().info(f"Station {self.target_station_index} processing complete.")
                     ...
                     if self.current_sequence_index < len(self.station_sequence):
                          ...
                          self.get_logger().info(f"Proceeding to next station: {self.target_station_index} ({next_station_name})")
            ...
            self.get_logger().info(f"Arrived back at Pickup Station (Index 0). Order '{self.current_order['order_name']}' complete.")
            ...
            if pickup_field:
                ...
                self.get_logger().warn("No Airtable field mapped for Pickup Station (Index 0). Cannot update status.")
...
def main(args=None):
    ...
    try:
        ...
        if pancake_robot_node.state not in [RobotState.GPIO_ERROR, RobotState.CAMERA_ERROR, RobotState.AIRTABLE_ERROR]:
            ...
            pancake_robot_node.get_logger().fatal(f"Node initialization failed with state: {pancake_robot_node.state.name}. Shutting down.")
