...
# --- NEW: Asymmetric Line Search Parameters (TUNABLE) ---
LOST_LINE_ROTATE_SPEED = (
    0.15  # rad/s - Speed for rotation when line is lost (tune this)
)
RIGHT_SEARCH_DURATION_SEC = 1.8  # How long to search right when lost (tune this)
LEFT_SEARCH_DURATION_SEC = (
    0.9  # How long to search left (should be < RIGHT_SEARCH_DURATION_SEC) (tune this)
)
# --- End Asymmetric Search Parameters ---
...
class PancakeRobotNode(Node):
    def __init__(self):
        ...
        # === NEW: Line Search State ===
        self.is_searching_line = (
            False  # Flag: True if currently performing search sweep
        )
        self.search_direction = "right"  # Current sweep direction ('right' or 'left')
        self.search_start_time = (
            0.0  # Timestamp when the current sweep direction started
        )
        # ============================
    ...
    def _init_ros2(self):
        ...
        try:
            ...
            # Check if action clients are ready (optional, add timeout)
            # self.drive_client.wait_for_server(timeout_sec=5.0)
            # self.rotate_client.wait_for_server(timeout_sec=5.0)
            # self.get_logger().info("Action clients connected.")
    ...
    def control_loop(self):
        ...
        if self.state in [
            ...
            self.stop_moving()
            # Optionally add periodic error logging or sound here
            # self.get_logger().error(f"System halted in state: {self.state.name}")
            return

        # --- Camera Frame Processing ---
        display_frame, mask_frame, color_detected = None, None, False
        # Only actively check for color detection when moving to a station
        ...
        if self.picam2 and self.state != RobotState.CAMERA_ERROR:
            try:
                ...
                # Check for station color only if we have a valid target
                if self.target_station_index != -1:
                    # check_for_station_color returns: detected_flag, display_frame, mask_frame
                    _detected_flag, display_frame, mask_frame = (
                        self.check_for_station_color(
                            raw_frame, self.target_station_index
                        )
                    )
                    # Only set the main color_detected flag if we are in the MOVING state
                    if process_color_flag:
                        color_detected = _detected_flag
                else:
                    # If no target, just prepare the raw frame for display
                    display_frame = raw_frame.copy()
                    # Add basic state text if no target-specific text was added
                    cv2.putText(
                        display_frame,
                        f"State: {self.state.name}",
                        (10, 30),
                    ...
                    # Ensure mask_frame exists even if color check wasn't run
                    mask_frame = np.zeros(
        ...
        try:
            ...
            if self.state == RobotState.IDLE:
                ...
                self.target_station_index = -1
                self.initial_line_found = False
                self.is_searching_line = False  # Reset search state

                # Try to fetch a new order
                self.current_order = self.fetch_order_from_airtable()

                if self.current_order:
                    # Check if fetch resulted in Airtable error state
                    ...
                    # No orders found, and no error occurred during fetch
                    self.state = RobotState.ALL_ORDERS_COMPLETE
                # If fetch_order set state to AIRTABLE_ERROR, loop will stop at the top check
            ...
            # State: PLANNING_ROUTE
                ...
                # Simplified planning: Add stations marked as WAITING in a fixed order
                # This assumes Cook 1 is always first if needed.
                ...
                # Add subsequent stations if they are WAITING
                ...
                # Always add Pickup station at the end
                self.station_sequence.append(
                    STATION_FIELD_TO_INDEX[AIRTABLE_PICKUP_STATUS_FIELD]
                )

                # Validation: Ensure the route is not empty (should at least have Pickup)
                if not self.station_sequence:
                    ...
                    self.is_searching_line = False  # Ensure search state is reset
            ...
            # State: MOVING_TO_STATION
                ...
                if color_detected:
                    ...
                    self.stop_moving()
                    # === Reset Search State on Arrival ===
                    self.is_searching_line = False
                    # =====================================
                ...
                if (
                    self.target_station_index == -1
                    or self.current_sequence_index < 0  # Add check for negative index
                ...
                # --- Line Following and Search Logic ---
                left_on, right_on = self.read_ir_sensors()
                current_time = time.time()  # Get current time for search logic
                ...
                if not left_on and not right_on:
                    if not self.initial_line_found:
                        ...
                        )  # Simple rotate right to find
                    else:
                        # Line was found previously, but now lost -> Initiate/Continue Asymmetric Search
                        if not self.is_searching_line:
                            # Start a new search cycle
                            self.get_logger().warning(
                                "Line lost! Starting asymmetric search sweep (prioritizing right)."
                            )
                            self.play_sound([(330, 100)])  # Lost line sound
                            self.is_searching_line = True
                            self.search_direction = (
                                "right"  # Always start search to the right
                            )
                            ...
                            elapsed_search_time = current_time - self.search_start_time

                            if self.search_direction == "right":
                                if elapsed_search_time < RIGHT_SEARCH_DURATION_SEC:
                                    ...
                                    self.get_logger().warning(
                                        "Right search sweep finished, switching to LEFT."
                                    )
                                    self.play_sound(
                                        [(280, 100)]
                                    )  # Switch direction sound
                                ...
                                if elapsed_search_time < LEFT_SEARCH_DURATION_SEC:
                                    ...
                                    self.get_logger().warning(
                                        "Left search sweep finished, line not found. Resetting search state."
                                    )
                                    self.play_sound([(220, 200)])  # Search failed sound
                                    self.is_searching_line = False  # End current search cycle to restart right next time
                                    self.stop_moving()  # Pause briefly before next attempt

                # === CASE 2: At least one Sensor ON Line ===
                else:
                    # Line is found (or re-found)! Reset search state if we were searching.
                    if self.is_searching_line:
                        self.get_logger().info("Line re-acquired during search.")
                        self.play_sound([(660, 100)])  # Found line sound
                        self.is_searching_line = False  # Stop searching sweep
                    ...
                    # Case Both OFF handled above in the search logic

            # State: ARRIVED_AT_STATION
            elif self.state == RobotState.ARRIVED_AT_STATION:
                self.stop_moving()  # Ensure robot is stopped
                if (
                    self.current_sequence_index < 0
                    or self.current_sequence_index >= len(self.station_sequence)
                ):
                ...
                if self.update_station_status(
                    ...
                    self.wait_start_time = (
                        time.time()
                    )  # Record time when waiting starts
                    ...
                    # update_station_status already set state to AIRTABLE_ERROR if it failed
                    self.get_logger().error(
            ...
            # State: WAITING_FOR_STATION_COMPLETION
                ...
                if elapsed_wait_time > STATION_WAIT_TIMEOUT_SEC:
                    ...
                    return  # Go to timeout state handler

                # Poll Airtable periodically
                if time.time() - self._last_airtable_check_time >= AIRTABLE_POLL_RATE:
                    self._last_airtable_check_time = time.time()

                    # Validate index and field again before checking Airtable
                    if (
                        self.current_sequence_index < 0
                        or self.current_sequence_index >= len(self.station_sequence)
                    ):
                    ...
                    if self.wait_for_station_completion(
                        ...
                        # Check if this was the last station in the sequence
                        if self.current_sequence_index >= len(self.station_sequence):
                            ...
                            # Reset color detection cooldown for the station we just finished
                            self.last_color_detection_times[current_station_idx] = 0.0
                            # Initiate leaving sequence
                        ...
                        # State already set by wait_for_station_completion
                    else:
            ...
            # State: STATION_TIMED_OUT
                ...
                self.stop_moving()
                # Reset everything as if order was aborted/finished badly
                self.current_order = None
                self.station_sequence = []
                self.current_sequence_index = -1
                self.target_station_index = -1
            ...
            # State: AIRTABLE_ERROR (Handled at the top, but log here if somehow reached)
                ...
                self.stop_moving()
                # State remains AIRTABLE_ERROR, halting loop
        ...
        # --- Post-State Machine: Display Update ---
            ...
            if self.debug_windows:
                try:
                    ...
                    # Ensure mask frame is displayed even if blank
                    ...
                    else:  # If somehow mask_frame is None, show a blank image
                        blank_mask = np.zeros(
                    ...
                    # Log display errors but don't crash the main loop
                    self.get_logger().error(
                        f"Display update error: {e}", exc_info=False
                    )  # exc_info=False to avoid spamming logs
...
def main(args=None):
    ...
    try:
        ...
        # Proceed only if initialization was successful (no fatal hardware/ROS errors)
        if node.state not in [
            RobotState.GPIO_ERROR,
            RobotState.CAMERA_ERROR,
            RobotState.ERROR,
            RobotState.AIRTABLE_ERROR,  # Add check here too
        ]:
            executor = SingleThreadedExecutor()
            executor.add_node(node)
            node.get_logger().info("Starting ROS2 executor spin...")
            # Spin the executor, which runs the control_loop via the timer
            executor.spin()
        else:
            # Log the specific failure state if init failed
            node.get_logger().fatal(
                f"Node initialization failed with state: {node.state.name}. Aborting execution."
            )
            # Perform cleanup even if init failed partially
            if node:
        ...
        # Catch unexpected errors during node creation or executor setup/spin
        ...
        if node:
            ...
            node.stop_moving()  # Ensure robot is stopped
            ...
            node.cleanup_hardware()  # Cleanup camera and GPIO
        ...
        # Final ROS shutdown if it hasn't happened yet
        if rclpy.ok():
