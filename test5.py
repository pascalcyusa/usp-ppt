...
class PancakeRobotNode(Node):
    ...
    def control_loop(self):
        ...
        try:
            ...
            if self.state == RobotState.IDLE:
                ...
                if not left_on and not right_on:  # Both OFF line (LOW)
                    if not self.initial_line_found:
                        self.get_logger().info("Searching initial line (RIGHT)...")
                        ...
                        if int(now) % 3 != 2:
                            self.get_logger().debug("...biased search: RIGHT")
                            self.move_robot(0.0, LOST_LINE_ROTATE_SPEED)  # Turn Right
                        else:
                            self.get_logger().debug("...biased search: LEFT")
                ...
                if idx not in STATION_INDEX_TO_FIELD:
                    self.get_logger().error(f"ARRIVED: No field for index {idx}.")
                    self.state = RobotState.ERROR
                    return
                ...
                if time.time() - self._last_airtable_check_time >= AIRTABLE_POLL_RATE:
                    ...
                    if not field:
                        self.get_logger().error(f"WAITING: No field for index {idx}.")
                        self.state = RobotState.ERROR
                        return
            ...
            self.get_logger().error(f"State machine exception: {e}", exc_info=True)
            self.state = RobotState.ERROR
            self.stop_moving()
