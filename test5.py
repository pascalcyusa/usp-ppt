...
STATUS_WAITING = 0; STATUS_ARRIVED = 1; STATUS_DONE = 99
...
LEFT_IR_PIN = 16; RIGHT_IR_PIN = 18
...
COMMON_HSV_LOWER = (35, 100, 100); COMMON_HSV_UPPER = (85, 255, 255); COMMON_COLOR_BGR = (0, 255, 0)
STATION_COLORS_HSV = { idx: {"name": STATION_INDEX_TO_FIELD.get(idx, f"Station {idx}").replace(" Status", ""), "hsv_lower": COMMON_HSV_LOWER, "hsv_upper": COMMON_HSV_UPPER, "color_bgr": COMMON_COLOR_BGR} for idx in STATION_FIELD_TO_INDEX.values() }
...
class PancakeRobotNode(Node):
    def __init__(self):
        ...
        self._last_airtable_check_time = 0.0; self.initial_line_found = False
    ...
    def _init_hardware(self):
        try: # GPIO
            ...
            self.get_logger().info(f"GPIO ok (L{LEFT_IR_PIN},R{RIGHT_IR_PIN}). Expect {IR_LINE_DETECT_SIGNAL}=ON_LINE.")
        except Exception as e: self.get_logger().error(f"FATAL: GPIO init: {e}", exc_info=True); self.state = RobotState.GPIO_ERROR; return
        try: # Camera
            self.picam2 = Picamera2(); config = self.picam2.create_preview_configuration(main={"size": CAMERA_RESOLUTION}, transform=CAMERA_TRANSFORM); self.picam2.configure(config)
            self.picam2.set_controls({"AfMode": controls.AfModeEnum.Continuous, "LensPosition": 0.0}); self.picam2.start(); time.sleep(2)
            self.get_logger().info("Camera ok.")
            if self.debug_windows: cv2.namedWindow("Camera Feed"); cv2.namedWindow("Color Detection Mask")
        except Exception as e: self.get_logger().error(f"FATAL: Camera init: {e}", exc_info=True); self.cleanup_gpio(); self.state = RobotState.CAMERA_ERROR; return

    def _init_ros2(self):
        try:
            self.cmd_vel_pub = self.create_publisher(Twist, '/cmd_vel', 10); self.audio_publisher = self.create_publisher(AudioNoteVector, '/cmd_audio', 10)
            self.drive_client = ActionClient(self, DriveDistance, '/drive_distance'); self.rotate_client = ActionClient(self, RotateAngle, '/rotate_angle')
            if not self.audio_publisher: raise RuntimeError("Audio pub failed."); self.get_logger().info("ROS2 ok.")
        except Exception as e: self.get_logger().error(f"FATAL: ROS2 init: {e}", exc_info=True); self.state = RobotState.ERROR
    ...
    def check_for_station_color(self, frame, target_idx):
        ...
        target_bgr = COMMON_COLOR_BGR; lower = np.array(COMMON_HSV_LOWER); upper = np.array(COMMON_HSV_UPPER)
        try:
            hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV); mask = cv2.inRange(hsv, lower, upper)
            mask_frame = mask; pixels = cv2.countNonZero(mask)
            text = f"Target: {target_name} ({pixels} px)"; cv2.putText(display_frame, text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, target_bgr, 2)
            now = time.time()
            if pixels > COLOR_DETECTION_THRESHOLD and (now - self.last_color_detection_times.get(target_idx, 0.0) > COLOR_COOLDOWN_SEC):
                self.last_color_detection_times[target_idx] = now; detected_flag = True
        ...
        except cv2.error as e: self.get_logger().error(f"OpenCV color error: {e}"); return False, display_frame, np.zeros_like(mask_frame)
        except Exception as e: self.get_logger().error(f"Color detect error: {e}", exc_info=True); return False, display_frame, np.zeros_like(mask_frame)
    ...
    def play_sound(self, notes):
        ...
        msg = AudioNoteVector(); log_str = []
        for f, d in notes:
            if f<=0 or d<=0: self.get_logger().warning(f"Skip invalid note: {f}Hz,{d}ms"); continue
            n = AudioNote(); n.frequency=int(f); n.max_runtime=Duration(sec=int(d/1000),nanosec=int((d%1000)*1e6)); msg.notes.append(n); log_str.append(f"({f}Hz,{d}ms)")
        if not msg.notes: self.get_logger().warning("No valid notes."); return
        self.get_logger().info(f"Audio Cmd: {','.join(log_str)}")
        try: self.audio_publisher.publish(msg); self.get_logger().debug("Audio published.")
    ...
    def control_loop(self):
        ...
        if self.picam2 and self.state != RobotState.CAMERA_ERROR:
            try:
                ...
                if self.target_station_index != -1: _det, display_frame, mask_frame = self.check_for_station_color(raw, self.target_station_index);
                if process_color: color_detected = _det
                else: display_frame = raw.copy(); cv2.putText(display_frame, f"State: {self.state.name}", (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2); cv2.putText(display_frame, "Target: None", (10,60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
            except Exception as e: 
        ...
        try:
            ...
            if self.state == RobotState.IDLE:
                ...
                self.station_sequence = []; status = self.current_order["station_status"]
                if status.get(AIRTABLE_COOKING_1_STATUS_FIELD)==STATUS_WAITING: self.station_sequence.append(1)
                else: self.get_logger().error(f"Order invalid start."); self.state = RobotState.IDLE; self.current_order = None; return
                if status.get(AIRTABLE_COOKING_2_STATUS_FIELD)==STATUS_WAITING: self.station_sequence.append(2)
                if status.get(AIRTABLE_CHOCOLATE_CHIPS_STATUS_FIELD)==STATUS_WAITING: self.station_sequence.append(3)
                if status.get(AIRTABLE_WHIPPED_CREAM_STATUS_FIELD)==STATUS_WAITING: self.station_sequence.append(4)
                if status.get(AIRTABLE_SPRINKLES_STATUS_FIELD)==STATUS_WAITING: self.station_sequence.append(5)
                self.station_sequence.append(0) # Pickup
                if not self.station_sequence: self.get_logger().error("Route plan failed!"); self.state = RobotState.IDLE; self.current_order = None
                else:
                    self.current_sequence_index=0; self.target_station_index=self.station_sequence[0]; self.initial_line_found=False
                    self.get_logger().info(f"Route: {self.station_sequence}. Next: {self.target_station_index}")
                    self.state = RobotState.MOVING_TO_STATION; self.play_sound([(440,100), (550,100), (660,100)])

            elif self.state == RobotState.LEAVING_STATION:
                elapsed = time.time() - self.leaving_station_start_time
                if elapsed < LEAVING_STATION_DURATION_SEC: self.move_robot(BASE_DRIVE_SPEED, 0.0)
                else:
                    self.get_logger().info(f"Finished leaving. Moving to {self.target_station_index}.")
                    self.stop_moving(); self.initial_line_found = False; self.state = RobotState.MOVING_TO_STATION

            elif self.state == RobotState.MOVING_TO_STATION:
                if color_detected: self.get_logger().info(f"Color detected for {self.target_station_index}. Arriving."); self.play_sound([(523,100),(659,150)]); self.stop_moving(); self.state = RobotState.ARRIVED_AT_STATION; return
                if self.target_station_index==-1 or self.current_sequence_index >= len(self.station_sequence): self.get_logger().error(f"MOVING: Invalid target/index."); self.stop_moving(); self.state = RobotState.ERROR; return
                ...
                self.stop_moving()
                if self.current_sequence_index < 0 or self.current_sequence_index >= len(self.station_sequence): self.get_logger().error("ARRIVED: Invalid index."); self.state = RobotState.ERROR; return
                idx = self.station_sequence[self.current_sequence_index]
                if idx not in STATION_INDEX_TO_FIELD: self.get_logger().error(f"ARRIVED: No field for index {idx}."); self.state = RobotState.ERROR; return
                field = STATION_INDEX_TO_FIELD[idx]
                if self.update_station_status(self.current_order["record_id"], field, STATUS_ARRIVED):
                ...
                if time.time() - self._last_airtable_check_time >= AIRTABLE_POLL_RATE:
                    ...
                    idx = self.station_sequence[self.current_sequence_index]; field = STATION_INDEX_TO_FIELD.get(idx)
                    if not field: self.get_logger().error(f"WAITING: No field for index {idx}."); self.state = RobotState.ERROR; return
                    if self.wait_for_station_completion(self.current_order["record_id"], field):
                        self.get_logger().info(f"Station {idx} ({field}) DONE."); self.play_sound([(659,150),(784,200)])
                        ...
                        if self.current_sequence_index >= len(self.station_sequence): self.get_logger().info("All stations complete."); self.state = RobotState.ORDER_COMPLETE
                            ...
                            self.target_station_index=self.station_sequence[self.current_sequence_index]; self.get_logger().info(f"Leaving station {idx} before moving to {self.target_station_index}.")
                            self.leaving_station_start_time = time.time(); self.state = RobotState.LEAVING_STATION; self.last_color_detection_times[idx]=0.0
                ...
                self.stop_moving(); self.current_order = None; self.station_sequence = []; self.current_sequence_index = -1; self.target_station_index = -1; self.initial_line_found = False; self.state = RobotState.IDLE
                ...
                self.current_order = None; self.station_sequence = []; self.current_sequence_index = -1; self.target_station_index = -1; self.initial_line_found = False; self.state = RobotState.IDLE
